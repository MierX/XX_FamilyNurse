...
<body>
	<article class="page-container">
		<form class="form form-horizontal" id="form-article-add" method="post" action="<?php echo U('edit');?>">
			<input type="hidden" class="input-text" value="Notice" name="table" readonly="readonly" />
			<?php if(!empty($data["id"])): ?><div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">公告序号：</label>
					<div class="formControls col-xs-8 col-sm-9">
						<input type="text" class="input-text" onmouseover="this.style.border = '1px #DDDDDD solid'" value="<?php echo ($data["id"]); ?>" name="where[id]" readonly="readonly" />
					</div>
				</div><?php endif; ?>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">公告标题：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" onmouseover="this.style.border = '1px #DDDDDD solid'" value="<?php echo ($data["title"]); ?>" name="data[title]" />
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">作者：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" onmouseover="this.style.border = '1px #DDDDDD solid'" value="<?php echo (session('admin_name')); ?>" name="data[author]" readonly="readonly" />
				</div>
			</div>
			<?php if(!empty($data["hot"])): ?><div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">热度：</label>
					<div class="formControls col-xs-8 col-sm-9">
						<input type="text" class="input-text" onmouseover="this.style.border = '1px #DDDDDD solid'" value="<?php echo ($data["hot"]); ?>" readonly="readonly" />
					</div>
				</div><?php endif; ?>
			<?php if(!empty($data["status"])): ?><div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">公告状态：</label>
					<div class="formControls col-xs-8 col-sm-9">
						<?php if($data["status"] == 1): ?><input type="text" class="input-text" onmouseover="this.style.border = '1px #DDDDDD solid'" value="开启" readonly="readonly" />
						<?php else: ?>
							<input type="text" class="input-text" onmouseover="this.style.border = '1px #DDDDDD solid'" value="关闭" readonly="readonly" /><?php endif; ?>
					</div>
				</div><?php endif; ?>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">公告内容：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<script id="editor" name="data[content]" type="text/plain" style="width:100%;height:400px;"><?php echo ((isset($data["content"]) && ($data["content"] !== ""))?($data["content"]):请新编辑内容); ?></script>
				</div>
			</div>
			<div class="row cl">
				<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
					<button onClick="article_save_submit();" class="btn radius" type="submit">&nbsp;&nbsp;保存&nbsp;&nbsp;</button>
					<button onClick="layer_close();" class="btn radius" type="button">&nbsp;&nbsp;取消&nbsp;&nbsp;</button>
				</div>
			</div>
		</form>
	</article>
